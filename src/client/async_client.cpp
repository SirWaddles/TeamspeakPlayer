...
void client::handle_read_headers(const boost::system::error_code& err)
{
	if (!err)
	{
		...
		httpData.append(std::string((std::istreambuf_iterator<char>(&response_)), std::istreambuf_iterator<char>()));
...
std::string* client::GetRequestData(){
	return &httpData;
}

URLDetails client::SplitURL(std::string url){
	int proto = url.find("://") + 3;
	URLDetails urlD;
	urlD.access = url.substr(0, proto);
	int fSlash = url.find_first_of('/', proto);
	urlD.server = url.substr(proto, fSlash - proto);
	urlD.query = url.substr(fSlash, std::string::npos);
	return urlD;
}