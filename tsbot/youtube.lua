...
vlc = {}
vlc.access = "http"
vlc.path = "http://www.youtube.com/watch?v=O4eOQ3Wo0QI"

function vlc.readline()
	return readMainLine()
end

vlc.msg = {}

function vlc.msg.err(arg)
	printConsole("ERROR:")
	printConsole(arg .. "\n")
end

function vlc.msg.info(arg)
	printConsole("INFO:")
	printConsole(arg .. "\n")
end

function vlc.msg.warn(arg)
	printConsole("WARN:" .. arg .. "\n")
end

function vlc.msg.dbg(arg)
	printConsole("DBG:" .. arg .. "\n")
end

vlc.var = {}

function vlc.var.inherit(param, value)
  return 1080
end

vlc.strings = {}
function vlc.strings.resolve_xml_special_chars(str)
	str = string.gsub(str, "&lt;", "<")
	str = string.gsub(str, "&gt;", ">")
	str = string.gsub(str, "&amp;", "&")
	return str
...
function vlc.gethexchar(hex)
	return string.char(tonumber(hex, 16))
end

function vlc.strings.decode_uri(str)
	str = string.gsub(str, "%%(%x%x)", vlc.gethexchar)
	return str
end

lastParse = {}

function GDoFinalEVal()
	lastParse = parse()
	return lastParse[1].path
end
...
function SetYTURL(urlarg)
	vlc.path = urlarg
end

function memFromStream(obj)
	return readLineFromStream(obj.id)
end

function vlc.stream(url)
	local streamID = createLuaStream(url)
	local obj = {}
	obj.id = streamID
	obj.readline = memFromStream
	return obj
end
